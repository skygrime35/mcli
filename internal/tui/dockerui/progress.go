// internal/tui/dockerui/progress.go
package dockerui
...
type progressChanMsg struct {
	msg docker.ProgressMsg
	ok  bool
}

func waitForNext(ch <-chan docker.ProgressMsg) tea.Cmd {
	return func() tea.Msg {
		m, ok := <-ch
...
type progressScreen struct {
	title  string
	ch     <-chan docker.ProgressMsg
	lines  []string
	err    error
	closed bool
}

func newProgressScreen(title string, ch <-chan docker.ProgressMsg) tui.Screen {
